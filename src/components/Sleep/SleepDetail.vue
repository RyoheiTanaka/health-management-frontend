...
import { getSleepApi } from '@/apis/FitbitLog'
...
onBeforeMount(async () => {
  ...
  if (Object.keys(sleep.value).length == 0 || sleep.value.id != id) {
    sleep.value = await getSleepApi(id)
    series.value.push((sleep.value.duration / 36000000) * 100)
    convertTime.value = convertMilliseconds(sleep.value.duration)
  }
        ...
        <div class="lg:w-[50%]">
          <div class="flex flex-col">
            ...
            <div class="grid grid-cols-2 rounded-sm border-b border-stroke">
              ...
              <div class="p-2.5 xl:p-5">
                <p class="text-sm font-medium text-black xsm:text-base">{{ sleep.efficiency }}点</p>
              </div>
            </div>
          </div>
        </div>
